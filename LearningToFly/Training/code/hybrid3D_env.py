...
class Hybrid3DEnv(gym.Env):
    ...
    def get_observation_vector(self):
        ...
        # print('now', now, 'target', self.target)
    ...
    def reset(self):
        ...
        if mode == 0:
            ...
            self.target_vy = 0.0
            self.target_vz = 0.0
            self.target_x = self.np_random.uniform(low = 30.0, high = 30.0)
            self.target_y = self.np_random.uniform(low = -50.0, high = 50.0)
            self.target_z = self.np_random.uniform(low = -3.0, high = -1.0)
            self.target_orig = Quaternion(axis = [self.target_x, self.target_y, self.target_z], 
                        angle = 1)

        self.target = np.array([self.target_vx, self.target_vy, self.target_vz, 0])
        self.targetpos = np.array([self.target_x, self.target_y, self.target_z])
        # ipdb.set_trace()
        # print('============')
        #print('target', self.target, 'targetpos', self.targetpos)
    ...
    def compute_reward(self):
        ...
        vel_local = self.calc_local_velocity(self.rigid_body.rpy[2], self.rigid_body.velocity)
        vel_cost = 5.0 * np.square((vel_local - self.target[0:3])).sum()
        ...
        # target_orig = Quaternion(axis = [30,30,-1], angle = 1)
        #print('target_orig', target_orig.axis)
        #print('orientation', self.rigid_body.orientation.axis)
        #target_orig = self.target_orig 
        #quater_cost = 100*((target_orig.axis - self.rigid_body.orientation.axis)**2 ).sum()
        quater_cost = 0
        ...
        poserror = 0
        for i in [0,1,2]:
            poserror += (targetpos[i]-self.rigid_body.position[i])**2
        ...
        reward = (alive_bonus - control_cost - vel_cost - orientation_cost - omega_cost - I_cost - I_pos - quater_cost) * self.dt
    ...
    def compute_reward_org(self):
        ...
        # I_cost = 1e-3 * np.square(self.I_error).sum()
        I_cost = 0.0
        ...
        #targetvector = [0,0,-2]
        #for i in [0,1,2]:
        #    poserror += (targetvector[i]-self.rigid_body.position[i])**2
        #I_pos = 100.0*poserror 
        # print('position', self.rigid_body.position)

        reward = (alive_bonus - control_cost - vel_cost - orientation_cost - omega_cost - I_cost - I_pos) * self.dt
    ...
    def step(self, action):
        ...
        # apply aerodynamics
        lift, drag, aoa = self.wing.compute_aerodynamics(
        ...
        done = done or (np.linalg.norm(self.rigid_body.velocity) > 30)
        done = done or (np.linalg.norm(self.rigid_body.omega) > 5.0 * math.pi)
        #done = done or (abs(self.rigid_body.velocity_body[1]) > 2) - default
        done = done or (abs(self.rigid_body.velocity_body[1]) > 10)
        # print(self.rigid_body.position)
        # > 0.0 means below ground
        done = done or (abs(self.rigid_body.position[0]) > 35) # x
        done = done or (abs(self.rigid_body.position[1]) > 35) # y
        done = done or (self.rigid_body.position[2] > 0.1) # z
        ...
        # print('time', self.timesofar, 'reward', reward, 'position', self.rigid_body.position, 'target', self.target)
